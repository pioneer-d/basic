...
bId - 모든 글의 고유번호
bIndent - 답변 단계(-)갯수
bGroup - 어떤 id의 답변인지 알게 해주는 id값. 같은 글의 답변들은 모두 같다.
bStep - 몇번째 단계의 답변인지 알게해주는 값.

수정,삭제,답변,조회수 증가 완성

replyShape() - step이 높을 수록 '답변의 답변'이 반복된것. 이들보다 상위로 답변을 쓸 경우 이 '답변의 답변'들의 step은 올라가야 한다.
JSP MVC2패턴 게시판 만들기 완성.
