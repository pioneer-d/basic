...
먼저 배열의 각 원소는 메모리 상에서 연속되게 놓인다.

포인터의 덧셈에서 만약 int형이라면 +1할 때마다 4를 곱한(int가 4byte인 것을 감안)값이 주소값에 더해짐.

배열의 각 원소가 메모리 상에서 연속적으로 놓인다면, 포인터+1 == 배열+1(인덱스+1)과 같은 주소값을 가지게 되지 않을까?
정답은 파일 day13_1.c 참고.

더 나아가 배열의 (주소)시작값을 알고 있다면 포인터에 해당 시작값을 대입한 뒤 배열의 모든 주소값은 물론 '배열의 값' 또한 알아낼 수 있겠다.
day13_2.c 파일 참고
...
▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶
배열에 어떠한 연산자를 붙이지 않고 변수이름 그대로 출력해보면 해당 배열의 주소값(시작점)이 나오는데..
그렇다고 배열의 이름이 배열의 첫번째 원소를 가리키는 포인터라고 할 수는 없다.
배열의 이름을 사용하면 암묵적으로 첫번째 원소를 가리키는 포인터로 타입 변환이 된다.
(== sizeof나 &연산자를 사용하지 않는 경우에)

sizeof == 크기를 알려주는 연산자. sizeof();
이 연산자를 통해 확인할 수 있다.
	* 64비트의 컴퓨터에서 포인터는 8byte! 	(다른 컴퓨터는 몇?)


▶▶▶▶▶▶▶▶▶▶▶▶▶▶▶
사실 아래의 두 값은 같은 값이다.
	a[3]  == *(a+3)
[]연산자가 사용되면, 자동적으로 오른쪽 형태로 바뀌어 연산이 된다.
지금까지 배운 것을 토대로 이해가 되는 부분.

3[arr] == *(3+a) 이런 해괴망측한 형태도 말이 된다.. 


복습..


문제 3,4,5,6 이랑 진도 쭉 빼기
